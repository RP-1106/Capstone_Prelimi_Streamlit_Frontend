...
st.title("Record your transactions here 💳 ")
...
def centered_button(col, label, image_path, key, color="black"):
    ...
    with stylable_container(
        ...
        css_styles=f"""
            .button-wrapper {{
                width: 50%; /* Takes full column width */
                display: flex;
                align-items: center;
...
def get_unique_values(df, column):
    if df is not None and column in df.columns:  # Check if DataFrame exists and column is present
        return df[column].unique().tolist()
    return []  # Return empty list if DataFrame or column doesn't exist

# Function to add a new row to the shared DataFrame
def add_new_transaction(df, new_transaction):
    if df is None:
        return pd.DataFrame([new_transaction])  # Create a new DataFrame if it doesn't exist
    else:
