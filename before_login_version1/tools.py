...
with col2:
    ...
    st.markdown("<div class='slider-label'>Loan Amount (<span class='code-style'> ₹ </span>)</div>", unsafe_allow_html=True)
    principal = st.slider("", min_value=0, max_value=1000000, value=1000, step=1, key="loan_amt")  # Default: 1000

    st.markdown("<div class='slider-label'>Rate of Interest (<span class='code-style'>% p.a.</span>)</div>", unsafe_allow_html=True)
    rate = st.slider("", min_value=0.0, max_value=30.0, value=3.0, step=0.1, key="interest")  # Default: 3.0

    st.markdown("<div class='slider-label'>Loan Tenure (<span class='code-style'>Years</span>)</div>", unsafe_allow_html=True)
